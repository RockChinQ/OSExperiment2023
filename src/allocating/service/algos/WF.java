...
public class WF extends AbstractAllocatingAlgorithm {
    @Override
    ...
    public void addProcess(int memorySize, RunningProcess process) {
        ...
        for (ProcessOccupancy occupancy : processOccupancies) {
            System.out.println("set 1 "+occupancy.getStart() + " - " + occupancy.getEnd());
        ...
        System.out.println("空闲空间：");
        for (Space space : spaces) {
            System.out.println(space.getStart() + " - " + space.getEnd());
        }

        // 找到空间大于 process.getNeedSize() 的最大的空间块
        // 倒序
        spaces.sort(Comparator.comparingInt(Space::getSize).reversed());

        boolean allocated = false;
        for (Space space : spaces) {
            if (space.getSize() >= process.getNeedSize()) {
                System.out.println(this.getAlgoName()+"分配空间：" + space.getStart() + " - " + space.getEnd());
